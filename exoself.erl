...
prep(Agent_Id,PM_PId,OpMode)->
	...
	%% Send initialization confirmation for live trading mode
	case OpMode of
		live_trading ->
			S#state.pm_pid ! {self(), initialized};
		_ ->
			ok
	end,
...
loop(S,live_trading)->
	receive
		{Cx_PId,evaluation_completed,Fitness,Cycles,Time,GoalReachedFlag}->
			%Don't terminate in live trading mode, just log the result and continue
			io:format("Live trading cycle complete, agent:~p continuing. Fitness:~p~n TotCycles:~p~n TimeAcc:~p Goal:~p~n",[self(),Fitness,Cycles,Time,GoalReachedFlag]),
			...
			%Reactivate cortex for next trading cycle
			Cx_PId ! {self(),reactivate},
			exoself:loop(S,live_trading);
		{stop_live_trading} ->
			%Graceful shutdown for live trading
			terminate_phenotype(S#state.cx_pid,S#state.spids,S#state.npids,S#state.apids,S#state.scape_pids,S#state.cpp_pids,S#state.cep_pids,S#state.substrate_pid),
			io:format("Live trading stopped for agent:~p~n",[self()]),
			S#state.pm_pid ! {self(),live_trading_stopped,S#state.specie_id}
	end.
