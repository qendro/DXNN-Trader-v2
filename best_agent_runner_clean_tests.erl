%% Test module for best_agent_runner_clean result formatting and reporting functions
-module(best_agent_runner_clean_tests).
-compile(export_all).
-include("records.hrl").

%% Test record definitions (matching best_agent_runner_clean.erl)
...
%% Main test runner
run_all_tests() ->
    io:format("~n=== RUNNING RESULT FORMATTING TESTS (CLEAN VERSION) ===~n"),
    ...
    Passed = length([R || {ok, _} = R <- Results]),
    Failed = length([R || {error, _, _, _} = R <- Results]),
...
test_format_results_basic() ->
    ...
    FormattedResults = best_agent_runner_clean:format_results(TestResults),
...
test_format_summary_results() ->
    ...
    SummaryConsole = best_agent_runner_clean:format_results(TestResults, #{format => summary, output => console}),
    SummaryString = best_agent_runner_clean:format_results(TestResults, #{format => summary, output => string}),
    SummaryNone = best_agent_runner_clean:format_results(TestResults, #{format => summary, output => none}),
...
test_format_detailed_results() ->
    ...
    DetailedResults = best_agent_runner_clean:format_results(TestResults, #{format => detailed, output => none}),
...
test_format_csv_results() ->
    ...
    % Test CSV format
    CSVResults = best_agent_runner_clean:format_results(TestResults, #{format => csv, output => string}),
...
test_format_json_results() ->
    ...
    % Test JSON format
    JSONResults = best_agent_runner_clean:format_results(TestResults, #{format => json, output => string}),
...
test_analyze_trading_decisions() ->
    ...
    % Analyze decisions
    Analysis = best_agent_runner_clean:analyze_trading_decisions(Decisions),
...
test_calculate_statistics() ->
    ...
    assert_equal(3.0, best_agent_runner_clean:calculate_median([1, 2, 3, 4, 5])),
    assert_equal(2.5, best_agent_runner_clean:calculate_median([1, 2, 3, 4])),
    assert_equal(1.0, best_agent_runner_clean:calculate_median([1])),
    
    % Test standard deviation calculation
    Values = [1, 2, 3, 4, 5],
    Mean = 3.0,
    StdDev = best_agent_runner_clean:calculate_std_dev(Values, Mean),
    assert_true(StdDev > 1.0 andalso StdDev < 2.0),  % Approximate check
    
    % Test quartiles calculation
    {Q1, Q2, Q3} = best_agent_runner_clean:calculate_quartiles([1, 2, 3, 4, 5, 6, 7, 8]),
...
test_profit_distribution_analysis() ->
    ...
    Analysis = best_agent_runner_clean:analyze_profit_distribution(Decisions),
...
test_signal_sequence_analysis() ->
    ...
    Analysis = best_agent_runner_clean:analyze_signal_sequences(Decisions),
...
test_time_pattern_analysis() ->
    ...
    Analysis = best_agent_runner_clean:analyze_time_patterns(Decisions),
...
test_decision_quality_assessment() ->
    ...
    % Test good decisions
    GoodQuality = best_agent_runner_clean:assess_decision_quality(GoodDecisions),
    ...
    % Test poor decisions
    PoorQuality = best_agent_runner_clean:assess_decision_quality(PoorDecisions),
...
test_file_output() ->
    ...
    TestResults = create_test_results(),
    TestFile = "test_results_output_clean.txt",
    
    % Test saving formatted results to file
    Result = best_agent_runner_clean:save_formatted_results_to_file(TestResults, TestFile, detailed),
    ...
    CSVFile = "test_results_output_clean.csv",
    CSVResult = best_agent_runner_clean:save_formatted_results_to_file(TestResults, CSVFile, csv),
...
test_edge_cases() ->
    ...
    EmptyFormatted = best_agent_runner_clean:format_results(EmptyResults, #{format => summary, output => none}),
    ...
    SingleFormatted = best_agent_runner_clean:format_results(SingleResults, #{format => detailed, output => none}),
    ...
    HoldFormatted = best_agent_runner_clean:format_results(HoldResults, #{format => summary, output => none}),
