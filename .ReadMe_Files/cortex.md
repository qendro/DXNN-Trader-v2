...
prep(ExoSelf_PId) ->
	...
	receive 
		{ExoSelf_PId,Id,SPIds,NPIds,APIds} ->
			put(start_time,now()),
			[SPId ! {self(),connectivity_check} || SPId <- SPIds],
			case receive {_From,connectivity_check_complete} -> ok after 5000 -> {error,timeout} end of
				ok -> 
					qlog:benchmarker(Id, io_lib:format("Cortex connectivity check complete: ~w | Exoself: ~w", [Id, ExoSelf_PId])),
					qlog:xLog(qStatus, "Cortex connectivity check complete: ~w | Exoself: ~w", [Id, ExoSelf_PId]),
					[SPId ! {self(),sync} || SPId <- SPIds], 
					loop(Id,ExoSelf_PId,SPIds,{APIds,APIds},NPIds,1,0,0,active);
				{error,timeout} -> 
					Cx = genotype:dirty_read({cortex,Id}),
					Agent_Id = Cx#cortex.agent_id,
					qlog:benchmarker(Id, io_lib:format("Cortex timed out after 5000ms | Agent ID: ~w | Cortex: ~w | Exoself: ~w", [Agent_Id, Id, ExoSelf_PId])),
					qlog:xLog(qStatus, "Cortex timed out after 5000ms | Agent ID: ~w | Cortex: ~w | Exoself: ~w", [Agent_Id, Id, ExoSelf_PId]),
					ExoSelf_PId ! {self(),evaluation_completed,0,99999999999999999999999,timer:now_diff(now(),get(start_time)),true}, 
					loop(Id,ExoSelf_PId,SPIds,{APIds,APIds},NPIds,0,0,0,inactive)
			end
	end.
...
loop(Id,ExoSelf_PId,SPIds,{[APId|APIds],MAPIds},NPIds,CycleAcc,FitnessAcc,EFAcc,active) ->
	receive 
		...
		{_From, connectivity_check_complete} ->
			loop(Id,ExoSelf_PId,SPIds,{APIds,MAPIds},NPIds,CycleAcc,FitnessAcc,EFAcc,active);
		terminate ->
